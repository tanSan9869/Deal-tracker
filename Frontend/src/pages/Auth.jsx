...
export default function Auth() {
  ...
  const handleAuth = async (e) => {
    ...
    try {
      if (isLogin) {
        ...
        // Save additional info if required based on your dealService
      }
      navigate("/dashboard");
    } catch (error) {
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      if (error) throw error;
      // Note: Redirect is handled automatically by Supabase for OAuth
    } catch (error) {
      ...
      <div className="w-full max-w-md bg-gray-800 border border-gray-700 p-8 rounded-2xl shadow-xl shadow-black/50">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-400 mt-2 text-sm">
            {isLogin
              ? "Enter your details to access your deals."
              : "Sign up to start tracking your deals today."}
          </p>
        </div>
        ...
        <form onSubmit={handleAuth} className="flex flex-col gap-5">
          ...
          <button
            type="submit"
            className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-semibold py-3 px-4 rounded-xl transition-all shadow-md shadow-indigo-600/20 mt-2"
          >
