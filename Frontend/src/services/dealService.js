...
const getAuthenticatedUserId = async () => {
  ...
  if (!user) throw new Error("You are not signed in. Please sign in again.");

  return user.id;
};

const buildBaseDealPayload = (deal, userId) => ({
  stage: deal.stage,
  status: deal.status,
  notes: deal.notes ?? "",
  user_id: userId,
...
export const addDeal = async (deal) => {
  ...
  const payload = {
    ...buildBaseDealPayload(deal, userId),
...
export const updateDeal = async (id, updates) => {
  ...
  if (typeof updates.startupName === "string") {
    payload.startup_name = updates.startupName;
...
export const saveUserProfile = async (userData) => {
  try {
    const { data: userDataRes } = await supabase.auth.getUser();
